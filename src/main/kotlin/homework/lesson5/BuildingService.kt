...
class BuildingService() {
 ...
 а цену преобразует в валюту (курс, можно создать константой) и возвращает
  новую коллекцию отсортированную по возрастанию цены.*/
    fun toLocalizedDescriptionSorted(Buildings: List<Building>, rate: Rates, language: Language): List<String> =
        Buildings.asSequence().sortedBy { it.price }.map {
            it.localizedDescription(language, rate)
        }.toList()
    ...
    fun returnFirstThreeArea(Buildings: List<Building>, threshold: Int)
            : List<String> =
