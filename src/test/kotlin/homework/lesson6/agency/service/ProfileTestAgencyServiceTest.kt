...
class ProfileTestAgencyServiceTest(
    ...
    fun findSoldPropertyByPrice(maxPrice: Int, pageNum: Int, pageSize: Int): List<Property> = mockMvc.get(
        "/soldProperty/find?maxPrice={maxPrice}&pageSize={pageSize}&pageNum={pageNum}", maxPrice, pageNum, pageSize
    ).readResponse()
    ...
    fun getStatusDeleteSoldPropertyById(id: Int): Int =
        mockMvc.delete("/soldProperty/{id}", id).andReturn().response.status
    ...
    private fun returnAddedRepository() {
        devSoldPropertiesDao.add(propertyTula)
        devSoldPropertiesDao.add(propertyChelyabinsk)
        devSoldPropertiesDao.add(propertySmolensk)
        devSoldPropertiesDao.add(propertyArkhangelsk)
    }
}